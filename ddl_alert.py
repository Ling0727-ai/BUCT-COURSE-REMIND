...
for cname, lid in hw_courses:
    ...
    for row in tsoup.select("table.valuelist tr")[1:]:
        ...
        deadline_parsed = re.match(r"(\d{4})年(\d{1,2})月(\d{1,2})日 (\d{2}):(\d{2}):(\d{2})", deadline)
        if deadline_parsed:
            deadline = time.mktime((int(deadline_parsed.group(1)), int(deadline_parsed.group(2)), int(deadline_parsed.group(3)),
                                     int(deadline_parsed.group(4)), int(deadline_parsed.group(5)), int(deadline_parsed.group(6)), 0, 0, 0))
        else:
            deadline = None

        # If deadline - current time <= 1 hr
        if deadline and (deadline - time.time() <= 3600) and (deadline - time.time() > 0):
            print(f"作业 '{title}' 的截止时间在1小时内，请尽快完成！")