...
def login():
    ...
    if not user:
        current_app.logger.warning(f"用户 {username_or_email} 登录失败：账号不存在")
        return jsonify({'error': '账号不存在，请检查用户名或邮箱'}), 401

    if not check_password_hash(user['password_hash'], password):
        current_app.logger.warning(f"用户 {username_or_email} 登录失败：密码错误")
    ...
    current_app.logger.info(
        f"用户 {user['username']} 登录成功（使用{'邮箱' if '@' in username_or_email else '用户名'}登录）")
...
def send_verification_email_enhanced(email, code, username=None):
        ...
        try:
            ...
            <body>
                <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
                    ...
                    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        ...
                        <p>您正在使用北化课程提醒系统的密码重置功能，您的验证码是：</p>
                        ...
                        {f'<p style="color: #666;">您的用户名：<strong>{username}</strong></p>' if username else '<p style="color: #999; font-size: 14px;">温馨提示：如果您忘记了用户名，请联系管理员 ***REMOVED***</p>'}
            ...
            if config['ssl']:
                ...
                try:
                    ...
                    current_app.logger.warning(f"STARTTLS失败，继续尝试: {starttls_err}")
...
def send_verification_code():
    ...
    try:
        ...
        try:
            if send_verification_email_enhanced(email, code, username):
                current_app.logger.info(f"验证码邮件发送成功: {email}")
