...
def send_verification_email(email, code):
    ...
    try:
        ...
        current_app.logger.info(f"✅ 验证码邮件已成功发送到 {email}")
        return True
...
def send_verification_code():
    ...
    try:
        ...
        # 直接使用修复后的邮件发送函数
        current_app.logger.info(f"尝试发送验证码邮件到: {email}")
        ...
        if send_verification_email(email, code):
            ...
            # 如果发送失败，返回测试模式响应
            current_app.logger.warning(f"邮件发送失败，返回测试模式响应")
            return jsonify({
                'message': '验证码已发送到您的邮箱',
                'test_mode': True,
                'verification_code': code,
                'note': '邮件服务暂时不可用，验证码已在响应中返回'
            })
