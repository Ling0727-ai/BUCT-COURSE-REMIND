    ...
    try:
        ...
        if existing:
            db[ASSIGNMENTS_COLLECTION].update_one(
                {'_id': existing['_id']},
                {'$set': {'publisher': publisher, 'content': content, 'updated_at': datetime.utcnow()}}
            )
            logger.info(f"更新作业: {subject} - {title}")
        else:
            assignment_data = {
                'subject': subject, 'title': title, 'content': content, 'due_date': due_date,
                'publisher': publisher, 'type': '作业', 'created_at': datetime.utcnow(), 'updated_at': datetime.utcnow()
            }
    ...
    try:
        ...
        if existing:
            db[TESTS_COLLECTION].update_one(
                {'_id': existing['_id']},
                {'$set': {'allowed_attempts': allowed_attempts, 'time_limit': time_limit, 'updated_at': datetime.utcnow()}}
            )
